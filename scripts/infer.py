...
def main():
    ...
    ap.add_argument("--model", required=True, help="Path to saved model (joblib).")
    ...
    ap.add_argument("--output", required=True, help="Path to save predictions CSV.")
    args = ap.parse_args()
    ...
    model = AirQualityModel.load(args.model)
    pred = model.predict(df, target=args.target)
    pred.to_csv(args.output, index=True)
